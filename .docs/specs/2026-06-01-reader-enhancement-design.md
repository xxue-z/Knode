...
```
UI 层
├── ReaderPage（改造：flutter_markdown + 高亮渲染 + 手势导航）
├── ContextToolbar（新建：浮动操作栏，9 个菜单项）
├── BookmarkPanel（新建：书签侧滑面板，左滑呼出）
├── NotePanel（新建：笔记侧滑面板，右滑呼出）
├── HeadingNavSheet（新建：标题导航底部弹窗，目录）
...
2. **可选**：通过 url_launcher 打开外部 AI 应用（DeepSeek app、豆包app、ChatGPT 等），同时复制到剪贴板
3. 设置中可配置默认 AI 助手

### 5.5 划重点/笔记流程

1. 点击"划重点/笔记" → 弹出样式选择器，展示用户在设置中预配置的所有高亮样式默认配置一套样式(背景色-黄色)
2. 选择样式后可选输入笔记内容
3. 保存到 highlights 表
...
侧滑面板实现方式：
- 使用 `Drawer` 或自定义 `SlidePanel`，从屏幕边缘滑入
- 面板宽度约为屏幕 75%，带半透明遮罩
- 点击遮罩或再次滑动关闭
...
- 列表展示：选中文字摘要、位置、日期
- 左滑单条可删除
- 点击书签 → 定位到保存位置，临时高亮闪烁 2 秒
- 空状态提示"暂无书签"

### 7.3 笔记列表面板（右滑呼出）

- 列表展示当前文档关联的所有高亮/笔记
- 每条显示：选中文字摘要、笔记内容预览、日期
- 点击跳转到对应位置
- 空状态提示"暂无笔记"
- 底部入口："查看笔记文档" → 打开关联的笔记文档

### 7.4 标题导航（目录）

阅读器工具栏新增"目录"图标，点击弹出文档标题列表：

- 从 Markdown AST 中提取 H1/H2/H3 标题
- 树形缩进展示（H1 顶格，H2 缩进一级，H3 缩进两级）
- 点击标题 → 定位到该标题在文档中的位置
- 当前可见区域的标题高亮标记
...
| packages/wiki/lib/screens/bookmark_panel.dart | 书签侧滑面板（左滑呼出） |
| packages/wiki/lib/screens/note_panel.dart | 笔记侧滑面板（右滑呼出） |
| packages/wiki/lib/screens/heading_nav_sheet.dart | 标题导航底部弹窗（目录） |
